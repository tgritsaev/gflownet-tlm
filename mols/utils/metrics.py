...
from mols.utils.utils import compute_trajs_prob, construct_batch
...
def compute_num_modes(rdmols, reward_thresh=7, tanimoto_thresh=0.7):
    # Candidate is pair (real_reward, RDMol)
    rdmols = sorted(rdmols, key=lambda m: m[0], reverse=True)
    # cut of candidates with reward less than reward_thresh
    candidates = [c for c in rdmols if c[0] >= reward_thresh]

    if len(candidates) == 0:
        return 0
    modes = [candidates[0]]
    modes_fps = [Chem.RDKFingerprint(candidates[0][1])]
    for i in range(1, len(candidates)):
        fp = Chem.RDKFingerprint(candidates[i][1])
        sim = DataStructs.BulkTanimotoSimilarity(fp, modes_fps)
...
def get_topk(rewards, k):
    ...
    Returns
    ----------
    average Topk rewards across all preferences
    """
...
def top_k_diversity(rdmols, r, K):
    x = []
    for i in np.argsort(r)[::-1]:
        y = Chem.RDKFingerprint(rdmols[i])
        if y is None:
            continue
        x.append(y)
        if len(x) >= K:
...
def monte_carlo_compute_correlation_stats(
    algo,
    ctx,
    task,
    ...
    sample_from_trained_backward: bool = False,
    n: int = 10,
    clip_log_R: float = -75,
):
    """
    Correct only for algo == TrajectoryBalance, as I utilize its `construct_batch`.
    TODO docstring
    """
    pred_log_rewards_list = []
    for _ in range(n):
        cond_info = task.sample_conditional_information(len(graphs), None)
        backward_trajs = algo.graph_sampler.sample_backward_from_graphs(
            [g.copy() for g in graphs],
            model if sample_from_trained_backward else None,
            cond_info["encoding"].to(device),
            0,
        )
        batch = construct_batch(ctx, backward_trajs, cond_info, log_rewards, do_parameterize_p_b)

        traj_log_p_F, traj_log_p_B, traj_unif_log_p_B = compute_trajs_prob(algo, model, batch.to(device))
        if sample_from_trained_backward:  # trained backward or uniform
            pred_log_rewards = (traj_log_p_F - traj_log_p_B).cpu()
        else:
            pred_log_rewards = (traj_log_p_F - traj_unif_log_p_B).cpu()
        pred_log_rewards[torch.isinf(pred_log_rewards)] = clip_log_R
        pred_log_rewards_list.append(pred_log_rewards)

    pred_log_rewards = torch.logsumexp(torch.stack(pred_log_rewards_list), dim=0)
    corr = np.corrcoef(pred_log_rewards, log_rewards)[0][1]

    return corr, 0.0  # the same as the np.mean and np.std
