...
from mols.algo.config import Backward, LossFN, NLoss, TBVariant
...
from mols.trainer import GFNAlgorithm
...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def __init__(
        ...
        assert self.do_predict_n or self.cfg.n_loss == NLoss.none, "`n_loss != NLoss.none` requires `do_predict_n`"
    ...
    def compute_batch_losses(
        ...
        model: TrajectoryBalanceModel,
        batch: gd.Batch,
        sampling_model: Optional[TrajectoryBalanceModel] = None,
    ):
        ...
        if self.do_parameterize_p_b:
            ...
            if sampling_model:
                with torch.no_grad():
                    target_fwd_cat, target_bck_cat, target_per_graph_out = sampling_model(batch, batched_cond_info)
        else:
            if self.model_is_autoregressive:
        ...
        if self.do_parameterize_p_b:
            ...
            bmask = 1 - batch.is_sink
            bmask[torch.isnan(log_p_B)] = 0
        ...
        if sampling_model:
            use_log_p_B = target_log_p_B
        else:
            use_log_p_B = log_p_B
        use_traj_log_p_B = scatter(use_log_p_B, batch_idx, dim=0, dim_size=num_trajs, reduce="sum")

        if self.cfg.variant == TBVariant.SubTB1:
            # SubTB interprets the per_graph_out predictions to predict the state flow F(s)
            if self.cfg.cum_subtb:
                traj_losses = self.subtb_cum(log_p_F, use_log_p_B, per_graph_out[:, 0], clip_log_R, batch.traj_lens)
            else:
                traj_losses = self.subtb_loss_fast(
                    log_p_F, use_log_p_B, per_graph_out[:, 0], clip_log_R, batch.traj_lens
                )
            ...
            transition_losses = self._loss(F_sn + log_p_F - F_sm - use_log_p_B)
            ...
            denominator = clip_log_R + use_traj_log_p_B
