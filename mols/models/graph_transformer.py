...
from mols.envs.graph_building_env import GraphActionCategorical, GraphActionType, action_type_to_mask
...
class GraphTransformerGFN(nn.Module):
    ...
    def __init__(
        ...
        for atype in chain(env_ctx.action_type_order, env_ctx.bck_action_type_order if do_bck else []):
            ...
            if unif_init and "remove" in atype.cname:
                mlps[atype.cname][-1].weight.data.fill_(0.0)
                mlps[atype.cname][-1].bias.data.fill_(0.0)
