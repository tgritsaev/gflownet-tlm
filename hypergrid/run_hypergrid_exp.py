...
# --algo experiments/config/algo.py:soft_ql --algo.munchausen.alpha 0.00
#
...
def main(_):
    ...
    env = HyperGrid(
        env_args.ndim, env_args.height, env_args.R0, env_args.R1, env_args.R2, device_str=general_args.device
    )
    env_name = f"{env_args.reward_type}_{env_args.ndim}_{env_args.height}"

    os.makedirs(f"grid_results_{env_name}", exist_ok=True)  # Create a folder with experiment results
