...
class GFNAlgorithm:
    ...
    def compute_batch_losses(
        self, model: nn.Module, batch: gd.Batch, sampling_model: Optional[nn.Module] = None
    ...
    def get_random_action_prob(self, it: int):
        ...
        if self.global_cfg.algo.train_det_after is None or it < self.global_cfg.algo.train_det_after:
            frac_it = it / self.global_cfg.num_training_steps
            if self.global_cfg.algo.train_random_action_prob_annealing_policy == "":
                multiplicator = 1
            elif self.global_cfg.algo.train_random_action_prob_annealing_policy == "linear":
                multiplicator = 1 - frac_it
            elif self.global_cfg.algo.train_random_action_prob_annealing_policy == "exp":
                multiplicator = self.global_cfg.algo.train_random_action_prob_exp**frac_it
            else:
                raise KeyError

            return multiplicator * self.global_cfg.algo.train_random_action_prob
        return 0
