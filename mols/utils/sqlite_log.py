...
def read_all_results(path):
    ...
    num_workers = len([f for f in os.listdir(path) if f.startswith("generated_objs") and f.endswith("db")])
    dfs = [
        pd.read_sql_query(f"SELECT * FROM results", sqlite3.connect(f"{path}/generated_objs_{i}.db"))
        for i in range(num_workers)
    ]
    return pd.concat(dfs).sort_index().reset_index(drop=True)


def read_all_results_at_least_reward(path, reward):
    # E402: module level import not at top of file, but pandas is an optional dependency
    import pandas as pd  # noqa: E402
    ...
    dfs = [
        pd.read_sql_query(
            f"SELECT * FROM results WHERE r >= {reward}", sqlite3.connect(f"{path}/generated_objs_{i}.db")
        )
        for i in range(num_workers)
    ]
    return pd.concat(dfs).sort_index().reset_index(drop=True)
...
def clear_all_data(db_path):
    try:
        ...
        for table_name in tables:
            ...
            # print(f"Cleared data from table: {table_name[0]}")
