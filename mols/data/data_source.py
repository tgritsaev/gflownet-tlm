...
from mols.data.replay_buffer import ReplayBuffer, detach_and_cpu
from mols.envs.graph_building_env import GraphBuildingEnvContext
from mols.utils.misc import get_worker_rng
...
class DataSource(IterableDataset):
    ...
    def get_model_samples(self, model, num_samples, num_new_replay_samples=None):
        if num_new_replay_samples is not None:
            assert self.replay_buffer is not None, "num_new_replay_samples specified without a replay buffer"
        if num_new_replay_samples is None:
            assert self.replay_buffer is None, "num_new_replay_samples not specified with a replay buffer"
    ...
    def create_batch(self, trajs, batch_info):
        ...
        if self.ctx.has_n() and self.cfg.algo.do_predict_n:
    ...
    def compute_properties(self, trajs, mark_as_online=False):
        ...
        if valid_idcs.shape[0] > 0 and obj_props.shape[0] > 0:
            # print(f"{all_fr}=")
            # print(f"{valid_idcs}=")
            # print(f"{obj_props}=")
