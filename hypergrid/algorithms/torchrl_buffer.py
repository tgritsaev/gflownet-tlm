...
from gfn.containers import Transitions, Trajectories
...
class TorchRLReplayBuffer:
    ...
    def __init__(
        ...
        if prioritized:
            ...
            self.replay_buffer = TensorDictReplayBuffer(storage=LazyTensorStorage(replay_buffer_size), batch_size=batch_size)
    ...
    def add_trajectories(self, trajectories: Trajectories, td_error=None):
        print(trajectories)
        self.replay_buffer.extend(
            TensorDict(
                {"states": trajectories.states.tensor},
                batch_size=16,
            )
        )
    ...
    def update_beta(self, progress: float) -> None:
        add_beta = (1.0 - self.initial_beta) * progress
...
def tensor_dict2t(env: Env, transitions_dict: TensorDict) -> Transitions:
    ...
    return Transitions(env=env, states=states, actions=actions, is_done=is_done, next_states=next_states)
