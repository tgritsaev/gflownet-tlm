...
from mols import LinScalar, LogScalar, ObjectProperties
from mols.config import Config
from mols.utils import metrics
from mols.utils.focus_model import TabularFocusModel
from mols.utils.misc import get_worker_device, get_worker_rng
from mols.utils.transforms import thermometer
...
class TemperatureConditional(Conditional[LogScalar, LogScalar]):
    ...
    def transform(self, cond_info: Dict[str, Tensor], logreward: LogScalar) -> LogScalar:
        assert len(logreward.shape) == len(cond_info["beta"].shape), f"dangerous shape mismatch: {logreward.shape} vs {cond_info['beta'].shape}"
        return LogScalar(logreward * cond_info["beta"])

    def reverse_transform(self, beta: Tensor, logreward: LogScalar):
        assert len(logreward.shape) == len(beta.shape), f"dangerous shape mismatch: {logreward.shape} vs {beta.shape}"
        return torch.exp(logreward / beta)
...
class FocusRegionConditional(Conditional[tuple[ObjectProperties, LogScalar], LogScalar]):
    ...
    def transform(self, cond_info: Dict[str, Tensor], data: tuple[ObjectProperties, LogScalar]) -> LogScalar:
        ...
        focus_coef, in_focus_mask = metrics.compute_focus_coef(flat_rewards, cond_info["focus_dir"], self.cfg.focus_cosim, self.cfg.focus_limit_coef)
